...
for idXMesh=1:round(Nt/Nplot):Nt
    ...
    daspect([1 1 1]); axis tight;  hold off
...
for i=1:round(numel(tauSave_Advect)/Nplot):numel(tauSave_Advect)
    ...
    title(sprintf('t = %.2f',t_i));daspect([1 1 1]); axis tight off
...
[bFTLE,detdelF_vert,eigVec] = FTLECompute(x0,y0,z0,xf,yf,zf,xfData,yfData,zfData,Tri_pts_Uni,Trif_Data);
...
hold on; cmocean('balance','pivot',0); 
scatter3(squeeze(xt_Advect_for(end,:)),squeeze(yt_Advect_for(end,:)),squeeze(zt_Advect_for(end,:)),10,'r','filled')
colorbar; axis equal; shading interp;  
title('abs(detdelF)-bw','Interpreter','latex'); set(gca,'FontSize',fntSz);

subplot(2,2,2)
trisurf(Tri_pts_Uni,x0,y0,z0,log(bFTLE),'FaceAlpha',1,'Edgecolor','none'); hold on 
quiver3(x0,y0,z0,eigVec(1,:),eigVec(2,:),eigVec(3,:),'r','ShowArrowHead','off'); hold off
colorbar; axis equal; shading interp;  
title('bwFTLE','Interpreter','latex'); set(gca,'FontSize',fntSz);
...
[fFTLE,detdelF_vert,eigVec] = FTLECompute(x0,y0,z0,xf,yf,zf,xfData,yfData,zfData,Tri_pts_Uni,Trif_Data);

subplot(2,2,3)
trisurf(Tri_pts_Uni,x0,y0,z0,log(detdelF_vert),'FaceAlpha',1,'Edgecolor','none');
cmocean('balance','pivot',0);colorbar; axis equal; shading interp;  
title('abs(detdelF)-fw','Interpreter','latex'); set(gca,'FontSize',fntSz);

subplot(2,2,4)
trisurf(Tri_pts_Uni,x0,y0,z0,log(fFTLE),'FaceAlpha',1,'Edgecolor','none');hold on 
quiver3(x0,y0,z0,eigVec(1,:),eigVec(2,:),eigVec(3,:),'r','ShowArrowHead','off'); hold off
colorbar; axis equal; shading interp;  
title('fwFTLE','Interpreter','latex'); set(gca,'FontSize',fntSz);