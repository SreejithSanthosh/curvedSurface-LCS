function [FTLE_vert,detdelF_vert,eig_vert] = FTLEComputev3(x0,y0,z0,xf,yf,zf,xfData,yfData,zfData,Tri_q0,Tri_qf)
% v3 is last updataed on 5/29/24
...
for i = 1:Nvert
    ...
    FTLE_vert(i) = sqrt(max(diag(e))); detdelF_vert(i) = abs(det(delF));
    [~,I] = sort(diag(e)); V = V(:,I); eigMax = zeros(3,1); eigMax(1:2) = V(:,2);
    eig_vert(:,i) = rotMatr0'*eigMax;

    if log(abs(det(delF)))<-10
        disp('Hey')
    end 
end 
